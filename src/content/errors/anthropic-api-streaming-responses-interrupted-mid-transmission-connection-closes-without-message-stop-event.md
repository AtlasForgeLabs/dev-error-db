---
title: "Anthropic API streaming connection closes without message_stop event during tool_use"
description: "Fix Anthropic API streaming responses that end abruptly without message_stop event when using tool_use with large JSON payloads Includes evidence for Anthropic API troubleshooting demand."
category: "Anthropic API"
technology: "Anthropic API"
error_signature: "Streaming responses interrupted mid-transmission — connection closes without message_stop event"
common_causes:
...
related_errors:
  - "Anthropic streaming response incomplete"
  - "Anthropic API stream disconnects unexpectedly"
  - "message_stop event not received Anthropic"
updated: "2026-05-12"
...
Evidence note: GitHub issue #842 (created 2025-11-11, 12 comments). Streaming from Anthropic API is prematurely terminated without message_stop event. Happens specifically with tool_use and large JSON payloads. SDK v0.68.0, Node.js/NestJS. Affects claude-3-5-haiku-latest and other models.
...
Start with the exact `Streaming responses interrupted mid-transmission — connection closes without message_stop event` text and the smallest action that reproduces it.
...
Rerun the same command, editor action, or request. The fix is working when that action completes without `Streaming responses interrupted mid-transmission — connection closes without message_stop event`.
